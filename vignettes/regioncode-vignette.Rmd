...
The package is developed to conquer such difficulties to match regional data across years more conveniently and correctly. In the current version, `regioncode` enables seamlessly converting Chinese prefecture regions'(named '地级市' in Chinese) formal names, common-used names, and division codes between each other and across thirty-four years from 1986 to 2019.
...
We randomly drew a eighteen-line sample.
...
Users needs to change the `method` argument to "name2code" or "name2name" to achieve the these conversions.
...
Future versions aims to adding more administrative level choice, from province level to county level. 
