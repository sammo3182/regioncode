...
# Derive the covered year range from the package's internal lookup table.
vec_yearRange <- utils::getFromNamespace("region_data", "regioncode") |>
  names() |>
...
To handle this type of data, `regioncode` provides the `incomplete_name` argument with four options:

- `"none"` (default): full official names are used for both input and output;
- `"from"`: the input data contains short names instead of full official names;
- `"to"`: the output will be short names rather than full official names;
- `"both"`: both input and output use short names.

In the following example, we shorten 70% of the city names in the input vector to demonstrate how `regioncode` resolves this issue using `incomplete_name = "from"`:
...
This functionality uses an internal lookup table covering all Chinese provinces, prefecture-level cities, and administrative area names, providing accurate and consistent romanization including conventional English names for regions such as Inner Mongolia, Tibet, Hong Kong, and Macao.
