...
We added a column of short names of the prefectures to further illustrate how the software works.

```{r toy, echo=TRUE, warning= FALSE, message=FALSE}
library(regioncode)
library(dplyr)
...
The current version enables to convert any pair of them mutually:
...
```{r code2code, echo=TRUE, warning= FALSE, message=FALSE}
...
In some cases, the original data may only have division codes or region names, but users need the other form or both formats of data. 
...
```{r code2name, echo=TRUE, warning= FALSE, message=FALSE}
regioncode(data_input = corruption$prefecture_id, 
           year_from = 2019,
           year_to = 1999, 
           method = "2name")
...
```{r name2code, echo=TRUE, warning= FALSE, message=FALSE}
...
```{r incompleteName, echo=TRUE, warning= FALSE, message=FALSE}
# Full, official names
corruption$prefecture
...
#### Municipalities
Another advanced application involves the case when the municipalities directly under the central government ("zhixiashi" in Chinese Pinyin).
...
# In the sample data, the division code of municipalities were coded
# as NA. Filling the codes of municipalities with their provinces' 
# codes.
code_zhixiashi <- c("110000", "120000", "310000", "400000")

corruption <- corruption %>% 
  mutate(prefecture_id = ifelse(province_id %in% code_zhixiashi, 
                                province_id, 
                                prefecture_id))
...
```{r topinyin, echo=TRUE, warning= FALSE, message=FALSE}
regioncode(data_input = corruption$prefecture, 
           year_from = 2019,
           year_to = 1999, 
           province = F,
...
```{r language_zone, echo=TRUE, warning= FALSE, message=FALSE}
regioncode(data_input = corruption$prefecture, 
           year_from = 2019,
           year_to = 1999, 
           province = F,
...
## Conclusion
`regioncode` provides a convenient way to convert Chinese administrative division codes, official names, and common-used short names between each other. 
