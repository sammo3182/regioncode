...
Some users may notice from the examples in this vignette that `regioncode` can output some provincially-administered counties ("直辖县"). 
We include them to avoid too much missingness, but administratively they are still county-level units.
Currently, we don't have the resource to cover all such counties and leave an issue for future [todo](https://github.com/sammo3182/regioncode/issues/54). 
If any users who would like promote fixing the issue, welcome to conduct a pull request or contact us.
]
